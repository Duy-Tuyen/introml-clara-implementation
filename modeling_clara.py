...
class CLaRa(PreTrainedModel):
    ...
    def _create_decoder(self, cfg: CLaRaConfig) -> AutoModelForCausalLM:
        ...
        if cfg.quantization == "no":
            ...
            return self._load_quantized_decoder(cfg, quant_config)
    ...
    @staticmethod
    def _load_quantized_decoder(cfg, quant_config):
        """Load decoder with bitsandbytes quantization, patching dispatch_model
        to handle the .to() incompatibility with quantized models.
        
        accelerate's dispatch_model calls model.to(device) for single-GPU maps,
        but .to() is not supported for 4-bit/8-bit bitsandbytes models (they are
        already on the correct device after quantization). This is a known compat
        issue with transformers==4.43.3 + accelerate on Kaggle.
        
        We patch the dispatch_model reference in BOTH accelerate.big_modeling
        AND transformers.modeling_utils, because transformers imports it into
        its own namespace via `from accelerate import dispatch_model`.
        """
        import accelerate.big_modeling
        import transformers.modeling_utils
        
        _orig_accel = accelerate.big_modeling.dispatch_model
        _orig_tfm   = transformers.modeling_utils.dispatch_model
        
        def _safe_dispatch(model, *args, **kwargs):
            try:
                return _orig_accel(model, *args, **kwargs)
            except ValueError as e:
                if ".to` is not supported" in str(e):
                    # dispatch_model tried model.to(device) which fails for 4-bit.
                    # Quantized linear layers are already on CUDA from bitsandbytes,
                    # but non-quantized params (embeddings, layernorms) may be on CPU.
                    # Move them manually.
        ...
        accelerate.big_modeling.dispatch_model = _safe_dispatch
        transformers.modeling_utils.dispatch_model = _safe_dispatch
        try:
            ...
            # Restore originals
            accelerate.big_modeling.dispatch_model = _orig_accel
            transformers.modeling_utils.dispatch_model = _orig_tfm
        return model
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, *args, **kwargs):
        ...
        map_location = torch.device("cpu")  # Always load to CPU first (4-bit compat)
        ...
        if config.lora:
            ...
            if os.path.exists(first_and_last_layers_path):
                ...
                for key in first_and_last_decoder_state_dict:
                    assert key in model.decoder.state_dict(), f"Key {key} not in decoder state_dict"
            ...
            # Ensure all non-quantized parameters are on CUDA.
            # decoder_first_last_layers.pth and adapters.pth are loaded to CPU
            # (map_location="cpu"), and load_state_dict copies them into the model
            # without moving to GPU. Move any remaining CPU params to CUDA.
